...
The purpose of the Launcher plugin is to provide a set of methods which allows an app to get the URL the client was opened with and clear the URL.
...
Get the URL which opened the client.

+ __successHandler__: will return the url as a string if one is set
+ __errorHandler__: will return a string if no url is set
...
Clears the URL used to open the client. Important to do so after consuming a launch URL so that your application doesn't process it more than once.
